...
Open **`commission-calculator.html`** in a browser. That's the whole install:
one file, no server, no network. It works offline and from a USB stick.

That file is the bundled build. To work on the source instead, open
`index.html`, which loads `assets/` alongside it. After changing anything in
`assets/`, rebuild the single-file version:
...
| File | What it holds |
| --- | --- |
| `commission-calculator.html` | The built single-file app — this is the one to use. Generated by `npm run build`; don't edit it directly. |
